...
export default{
    ...
    setup(){
        ...
        const title = ref('Share Transfer Details');
        const addButtonLabel = ref('New Share Transfer');
        const addingRight = ref('Adding Share Transfers');
        const removingRight = ref('Deleting Share Transfers');
        const rightsModule = ref('MMS');
        const idField = 'share_transfer_id';
        ...
        const actions = ref([
            {name: 'delete', icon: 'fa fa-trash', title: 'Delete Transfer', rightName: 'Deleting Share Transfers'},
        ])
        ...
        const importTransfers = () =>{
            store.commit('pageTab/ADD_PAGE', {'MMS':'Import_Share_Transfers'})
            store.state.pageTab.mmsActiveTab = 'Import_Share_Transfers';
        }
        ...
        const handleSelectedAccountTo = async(option) =>{
            await store.dispatch('Share_Accounts/handleSelectedAccount', option)
            accountToID.value = store.state.Share_Accounts.accountID;
        };
        ...
        const handleActionClick = async(rowIndex, action, row) =>{
            if(action == 'delete'){
                ...
                let formData = {
                    ...
                    share_transfer: transferID
                }
                await store.dispatch('Share_Transfers/deleteShareTransfer',formData).
                then(()=>{
                    searchTransfers();
                })
            }
        ...
        const createTransfer = async() =>{
            ...
            let formData = {
                ...
                share_account_from: accountID.value,
                share_account_from_id: accountID.value,
                share_account_to: accountToID.value,
                share_account_to_id: accountToID.value,
                journal: null,
                journal_id: null,
                user: userID.value,
                company: companyID.value
            ...
            if(errors.value.length){
                ...
                try {
                    const response = await store.dispatch('Share_Transfers/createShareTransfer', formData);
        ...
        const removeTransfer = async() =>{
            if(selectedIds.value.length == 1){
                let formData = {
                    company: companyID.value,
                    share_transfer: selectedIds.value,
                }
                try{
                    const response = await store.dispatch('Share_Transfers/deleteShareTransfer',formData)
        ...
        const searchTransfers = () =>{
            ...
            axios
            .post(`api/v1/share-transfers-search/?page=${currentPage.value}`,formData)
            .then((response)=>{
                transfersList.value = response.data.results;
                store.commit('Share_Transfers/LIST_TRANSFERS', transfersList.value)
