...
export default{
    ...
    setup(){
        ...
        const idField = 'loan_prepayment_alloc_id';
        ...
        const handleActionClick = async(rowIndex, action, row) =>{
            if(action == 'delete'){
                const allocationID = [row['loan_prepayment_alloc_id']];
                ...
                if(reversalStatus == "Yes"){
                    ...
                    let formData = {
                        ...
                        loan_prepayment_allocs: allocationID
                    }
                    await store.dispatch('Loan_Prepayment_Alloc/deleteAllocation',formData)
                    searchAllocations();
                }
        ...
        const removeAllocation = async() =>{
            if(selectedIds.value.length == 1){
                let formData = {
                    company: companyID.value,
                    loan_prepayment_allocs: selectedIds.value,
                }
                try{
                    const response = await store.dispatch('Loan_Prepayment_Alloc/deleteAllocation',formData)
        ...
        const searchAllocations = () =>{
            ...
            axios
            .post(`api/v1/loan-prepayment-allocations-search/?page=${currentPage.value}`,formData)
            .then((response)=>{
                prepaymentsList.value = response.data.results;
                store.commit('Loan_Prepayment_Alloc/LIST_PREPAYMENT_ALLOCATIONS', prepaymentsList.value)
