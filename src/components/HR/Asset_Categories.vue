...
export default{
    ...
    setup(){
        ...
        const addingRight = ref('Adding Asset Categories');
        const removingRight = ref('Deleting Asset Categories');
        const rightsModule = ref('HR');
        const idField = 'asset_category_id';
        ...
        const isEditing = computed(()=> store.state.Asset_Categories.isEditing)
        const selectedCategory = computed(()=> store.state.Asset_Categories.selectedCategory);
        const tableColumns = ref([
            {type: "checkbox"},
            {label: "Name", key: "category_name", type: "text", editable: false},
        ])
        const actions = ref([
            {name: 'edit', icon: 'fa fa-edit', title: 'Edit Category', rightName: 'Editing Asset Categories'},
            {name: 'delete', icon: 'fa fa-trash', title: 'Delete Category', rightName: 'Deleting Asset Categories'},
        ])
        ...
        const addNewCategory = () =>{
            ...
            handleReset();
            store.dispatch("Asset_Categories/updateState",{isEditing:false})
        ...
        const handleActionClick = async(rowIndex, action, row) =>{
            if( action == 'edit'){
                ...
                await store.dispatch('Asset_Categories/fetchAssetCategory',formData).
                then(()=>{
                    depModalVisible.value = true;
                    flex_basis.value = '1/2';
                    flex_basis_percentage.value = '50';
                ...
                await store.dispatch('Asset_Categories/deleteAssetCategory',formData).
                then(()=>{
                    searchCategories();
                })
            }
        ...
        const createCategory = async() =>{
            ...
            if(errors.value.length){
                ...
                try {
                    const response = await store.dispatch('Asset_Categories/createAssetCategory', formData);
        ...
        const updateCategory = async() =>{
            ...
            let formData = {
                category_name: formFields.value[0].value,
                module: selectedCategory.value.module,
                company: companyID.value,
                asset_category: selectedCategory.value.asset_category_id
            }
            for(let i=0; i < formFields.value.length; i++){
                if(formFields.value[i].value =='' && formFields.value[i].required == true){
                    errors.value.push('Error');
            ...
            if(errors.value.length){
                ...
                try {
                    const response = await store.dispatch('Asset_Categories/updateAssetCategory', formData);
        ...
        const removeCategory = async() =>{
            if(selectedIds.value.length == 1){
                let formData = {
                    company: companyID.value,
                    asset_category: selectedIds.value,
                }
                try{
                    const response = await store.dispatch('Asset_Categories/deleteAssetCategory',formData)
                    if(response && response.status == 200){
        ...
        const searchCategories = () =>{
            ...
            axios
            .post(`api/v1/asset-categories-search/?page=${currentPage.value}`,formData)
            .then((response)=>{
                categoryList.value = response.data.results;
                store.commit('Asset_Categories/LIST_ASSET_CATEGORIES', categoryList.value)
