...
export default{
    ...
    setup(){
        ...
        const idField = 'sms_template_id';
        const addButtonLabel = ref('New Template');
        const addingRight = ref('Creating SMS Template');
        ...
        const isEditing = computed(()=> store.state.SMS_Templates.isEditing);
        const selectedTemplate = computed(()=> store.state.SMS_Templates.selectedTemplate);
        ...
        const actions = ref([
            {name: 'edit', icon: 'fa fa-edit', title: 'Edit Template', rightName: 'Editing SMS Template'},
            {name: 'delete', icon: 'fa fa-trash', title: 'Delete Template', rightName: 'Deleting SMS Template'},
        ])
        const companyID = computed(()=> store.state.userData.company_id);
        const name_search = computed({
            get: () => store.state.SMS_Templates.name_search,
            set: (value) => store.commit('SMS_Templates/SET_SEARCH_FILTERS', {"name_search":value}),
        });
        const type_search = computed({
            get: () => store.state.SMS_Templates.type_search,
            set: (value) => store.commit('SMS_Templates/SET_SEARCH_FILTERS', {"type_search":value}),
        });
        ...
        const removeTemplate = async() =>{
            if(selectedIds.value.length == 1){
                let formData = {
                    ...
                    sms_template: selectedIds.value
                }
                try{
                    const response = await store.dispatch('SMS_Templates/deleteTemplate',formData)
        ...
        const searchTemplates = () =>{
            ...
            axios
            .post(`api/v1/sms-templates-search/?page=${currentPage.value}`,formData)
            .then((response)=>{
                templatesList.value = response.data.results;
                store.commit('SMS_Templates/LIST_TEMPLATES', templatesList.value)
        ...
        const resetFilters = () =>{
            store.commit('SMS_Templates/RESET_SEARCH_FILTERS')
            searchTemplates();
        }
        const loadPrev = () =>{
        ...
        const addNewTemplate = async() =>{
            await store.dispatch("SMS_Templates/updateState",{selectedTemplate:null, isEditing:false})
            store.commit('pageTab/ADD_PAGE', {'SET':'Design_SMS_Template'});
            store.state.pageTab.setActiveTab = 'Design_SMS_Template';    
            
        }
        const handleActionClick = async(rowIndex, action, row) =>{
            if( action == 'edit'){
                const templateID = row['sms_template_id'];
                let formData = {
                    company: companyID.value,
                    sms_template: templateID
                }

                await store.dispatch('SMS_Templates/fetchTemplate',formData)
                .then(()=>{
                    store.commit('pageTab/ADD_PAGE', {'SET':'Design_SMS_Template'});
                    store.state.pageTab.setActiveTab = 'Design_SMS_Template'; 
                })
                ...
                await store.dispatch('SMS_Templates/deleteTemplate',formData).
                then(()=>{
                    searchTemplates();
                })
            }else if(action == 'view'){
