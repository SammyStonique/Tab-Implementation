...
export default defineComponent({
    ...
    setup(){
        ...
        const receiptRows = computed(() => store.state.Journals.inventoryRcptList);
        ...
        const fetchCustomerInvoices = async() =>{
            if(customerID.value){
                await store.dispatch('Journals/fetchInventoryInvoices', {company:companyID.value, txn_type:["INV", "DBN"], customer:customerID.value, status:"Open"})
            }       
        };
        ...
        const handleReset = () =>{
            store.dispatch('Journals/updateState', {inventoryRcptList: [], outstandingBalance: 0})
