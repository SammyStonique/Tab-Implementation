...
export default defineComponent({
    ...
    setup(){
        ...
        const receiptRows = computed(() => store.state.Journals.inventoryPvList);
        ...
        const fetchVendorBills = async() =>{
            if(vendorID.value){
                await store.dispatch('Journals/fetchInventoryBills', {company:companyID.value, txn_type:["BIL"], customer:vendorID.value, status:"Open"})
            }       
        };
        ...
        const handleReset = () =>{
            store.dispatch('Journals/updateState', {inventoryPvList: [], outstandingBalance: 0})
